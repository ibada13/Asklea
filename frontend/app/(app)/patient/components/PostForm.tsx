...
import { UserField } from '../../admin/components/def/definitios';
...
const PostForm = ({
  ...
  const [formData, setFormData] = useState<Record<string, string>>(
    Object.fromEntries(userFields.map((f) => [f.name, initialData[f.name] ?? '']))
  );
...
const handleSubmit = async (e: React.FormEvent) => {
  ...
  try {
    ...
    console.log(err);
    const errorMsg = err?.response?.data?.detail || 'edit request was not sent';
    router.push(`${redirect}?msg=${errorMsg}&color=red`, { scroll: false });
  }
};
  ...
  return (
    ...
    >
      ...
      <div className="flex justify-end gap-4">
        <button
          type="reset"
          onClick={() => setFormData(Object.fromEntries(userFields.map((f) => [f.name, initialData[f.name] ?? ''])))}
